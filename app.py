...
HEADER_ROWS = [
    ...
    [""] * 9,
    ["S.No", "Inquired Part No", "Part Number", "Manf.Part", "Description", "Brand", "Stock on Hand", "Unit Price", "COO"]
]
MERGE_CELLS = [("A1", "I1"), ("A2", "B2"), ("C2", "E2"), ("A3", "B3"), ("C3", "E3"), ("F2", "G3"), ("H2", "I3")]
...
if uploaded_file:
    ...
    df = df[["S.No", "Inquired Part No", "Part Number", "Manf.Part", "Description", "Brand", "Stock on Hand", "Unit Price", "COO"]]

    wb = Workbook()
    ws1 = wb.active
    ws1.title = "Sheet1"
    apply_header(ws1)
    write_data(ws1, df[(df["Stock on Hand"] != 0) & (df["Brand"].notna())])
    adjust_column_widths(ws1)

    df_zero = df[df["Stock on Hand"] == 0]
    df_zero_filtered = df_zero[~df_zero["S.No"].isin(df[(df["Stock on Hand"] != 0) & (df["Brand"].notna())]["S.No"])]
